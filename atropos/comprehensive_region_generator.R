...
Pol_ttome = customimportbed6("/hpcnfs/data/GN2/gmandana/bin/4.1.0/home/ieo5559/Shannahan/denovo_promprox.bed")
...
systemTUs = augmented
systemTUs = readRDS("augmented_14Jul.RDS")
...
div = makeGRangesFromDataFrame(as.data.frame(systemTUs) %>% 
                                   ...
                                   dplyr::filter(n() > 1) %>%   ###must be > 1 tx per NDR
                                   dplyr::filter(length(unique(strand)) > 1) %>%   ##must be diff strand
                                   dplyr::filter(length(unique(iscoding)) > 1),   
                                 keep.extra.columns = T)
