...
customimportbed6 = function(fname){
  ...
  fname1 = gsub(fname, pattern = "\n", replacement = "")
  regions = makeGRangesFromDataFrame(read.table(fname1, header = F), 
...
repeatcounts = function(ranges, repeatsbed){
  ...
  df = do.call('cbind', lapply(families, function(x){
    countOverlaps(ranges, repeatsbed[repeatsbed$V4 == x], ignore.strand = T)
    }))
...
slop = function(regions, ext){
  
  slopped = resize(regions, width = width(regions) + (2*ext), fix = "center")
  peaks_s = reduce(slopped)
  return(peaks_s)
  
}

### START PEAKSPLITTER FUNCTIONS
peaksplitter = function(vector, winsize = 10, thresh = 0.07){
  
  #smooth profile before taking gradient
  ...
  peaks_filt = peaks_filt[lapply(peaks_filt, length) > 0]
  return(peaks_filt)
...
######### END PEAKSPLITTER FUNCTIONS

break_tx = function(range, bpoints){
  
  test = subsetByOverlaps(bpoints, range)
  if(length(test) == 0){
    return(range)
  }
  mcols(test) = NULL
  ...
  #set metadata to...
  mcols(test2)[2:length(test2),] = rep(mcols(test2[1,]), length(test2)-1)
  
  end(test2[1:length(test2)-1]) = start(test)
  names(test2) = paste0(names(range), "_", 1:length(test2))
